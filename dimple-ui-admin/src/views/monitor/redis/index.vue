    ...
    <el-table v-loading="keyDefineListLoad" :data="keyDefineList" row-key="id" @row-click="openKeyTemplate">
      ...
      <el-table-column prop="timeoutType" label="超时时间" width="200">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.redis_timeout_type" :value="scope.row.timeoutType"/>
        ...
        <el-col :span="10">
          <el-card :bordered="false" style="height: 70vh">
            <div slot="header">
              <span>缓存内容</span>
              <el-button style="float: right; padding: 3px 0" type="text" icon="el-icon-refresh-right"
                         @click="handleDeleteKeys(keyTemplate)">清理全部
              </el-button>
            </div>
...
import {
  ...
  getKeyValue,
  getRedisMonitorInfo
} from "@/api/monitor/redis";
import * as echarts from 'echarts'

require('echarts/theme/macarons') // echarts theme
export default {
  name: "Server",
  dicts: ['redis_timeout_type'],
  data() {
    return {
      // 统计命令信息
      commandstats: null,
  ...
  methods: {
    ...
    getList() {
      // 查询 Redis 监控信息
      getRedisMonitorInfo().then((response) => {
