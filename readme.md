...
# DimpleBlog
# 没错，我又在回来了，本次版本V1.2，正在开发中，如需稳定版请使用ReleaseV1.1版本，本次大规模更新预计结束时间8月中旬。期间代码不可用。请使用ReleaseV1.1版本。期间代码不可用。请使用ReleaseV1.1版本。期间代码不可用。请使用ReleaseV1.1版本。期间代码不可用。请使用ReleaseV1.1版本。
本次版本更新的目的在于构建一个简单的应用。代码方面力求精简。同时修复之前没来得及修复的bug。主要精力在于后台部门，前台不会做太大改动。
